...
//      COMPLETED (7) Generate a weather summary with the date, description, high and low
        // Weather Summary
        String weatherForThisDay = getWeatherSummary();
...
//      COMPLETED (8) Display the summary that you created above
    ...
    private String getWeatherSummary() {
        ...
        String friendlyDate = SunshineDateUtils
                .getFriendlyDateString(mContext, dateInMillis, false);

        // Weather max and min temperatures
        double maxTempInCelcius = mCursor.getFloat(MainActivity.INDEX_MAX_TEMP);
        double minTempInCelcius = mCursor.getFloat(MainActivity.INDEX_MIN_TEMP);
        String highAndLowTemp = SunshineWeatherUtils.formatHighLows(mContext, maxTempInCelcius, minTempInCelcius);

        // Weather Summary
        String weatherForThisDay = friendlyDate + " - " + weatherDescription + " - " + highAndLowTemp;

        return weatherForThisDay;
    }
